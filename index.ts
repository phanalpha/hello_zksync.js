...
run(async function (z) {
  const { connect, wallets, provider, log } = z;
  const program = new Command('r');

  program.command('deposit <amount> <wallets...>')
    .action(async function (amount, is) {
      for (const i of is) {
  ...
  program.command('deploy <name> <symbol> <wallet>')
    .action(async function (name, symbol, i) {
  ...
  program.command('mint <content-hash> <recipient> <creator>')
    .action(async function (content_hash, recipient, creator) {
  ...
  program.command('balance-of <address> <wallet>')
    .action(async function (address, i) {
  ...
  program.command('owner-of <address> <token>')
    .action(async function (address, token) {
      const contract = await connect(address);
  ...
  program.command('list <wallet> <token> <price>')
    .option('--bid')
    .action(async function (i, token, price, options) {
      await list(wallets[i], options.bid, +token, +price, z);
    });

  program.command('swap <submitter> <seller> <buyer> <token> <price>')
    .action(async function (submitter, seller, buyer, token, price) {
      await swap(wallets[submitter], wallets[seller], wallets[buyer], +token, +price, z);
    });
